...
//    if ($("#loginAdmin\\:email_m").val()) {
//        $("#loginAdmin\\:emailcheck_m").val("");
//    }
//});
...
function ChangeImageProduct() {
    document.getElementById("imgFileProduct").click();
}
...
function readProductURL(input, imgPath) {

    if (input.files && input.files[0]) {
        var reader = new FileReader();

        reader.onload = function (e) {
            $('#photoPreviewProduct').attr('src', e.target.result);
        };

        reader.readAsDataURL(input.files[0]);
    } else {
        $('#photoPreviewProduct').attr('src', imgPath);
    }
...
$(function () {
    $("#showPassword").click(function () {
        ...
        $("#loginAdmin\\:password").attr("type", type);
    });
});
...
$(document).ready(function () {
    var dateStart;
    var dateEnd;
    if ($('#dealForm\\:start').val()) {
        var fromStart = $('#dealForm\\:start').val().split("/");
        dateStart = new Date(fromStart[2], fromStart[1] - 1, fromStart[0]);
    } else {
        dateStart = new Date();
    }

    if ($('#dealForm\\:end').val()) {
        var fromStart = $('#dealForm\\:end').val().split("/");
        dateEnd = new Date(fromStart[2], fromStart[1] - 1, fromStart[0]);
    } else {
        dateEnd = new Date();
    }

    $("#dealForm\\:start").datepicker({
        format: 'dd/mm/yyyy',
        autoclose: true,
        todayHighlight: true
    }).datepicker('setDate', dateStart).datepicker('setStartDate', dateStart);

    $("#dealForm\\:end").datepicker({
        format: 'dd/mm/yyyy',
        autoclose: true,
        todayHighlight: true
    }).datepicker('setDate', dateEnd).datepicker('setStartDate', dateStart);
});

$('#dealForm\\:start').on("keydown", function () {
    $('#dealForm\\:start').blur();
});

$('#dealForm\\:start').on("change", function () {
    if (!$('#dealForm\\:end').val()) {
        $('#dealForm\\:end').val($('#dealForm\\:start').val());
        var from = $('#dealForm\\:start').val().split("/");
        $("#dealForm\\:end").datepicker({
            format: 'dd/mm/yyyy',
            autoclose: true,
            todayHighlight: true
        }).datepicker('setDate', new Date(from[2], from[1] - 1, from[0]));
    } else {
        var dateStart;
        var dateEnd;
        var fromStart = $('#dealForm\\:start').val().split("/");
...
$('#dealForm\\:end').on("keydown", function () {
    $('#dealForm\\:end').blur();
});

$(document).ready(function () {
    CKEDITOR.replace('productDescriptionEditor');
    CKEDITOR.instances['productDescriptionEditor'].setData($("#productDescription").val());
});

function SetDescription() {
    $("#productDescription").val(CKEDITOR.instances['productDescriptionEditor'].getData());
}
